...
$(document).ready(function() {
    ...
    //     '<a href="#!"><span class="shopping-cart-icon"></span></a>' +
    //     '</div>' +
    //     '<div class="style-block">' +
    //     '<h5 class="mb-2">購物車</h4>' +
    //     '<hr class="">' +
    ...
    $('.header-color > a').on('click', function() {
        ...
        if (temp == "header-default") {
            ...
            $('.m-header > .b-brand > .logo').attr('src', '/static/assets/images/logo-dark.png');
        } else {
            $('.pcoded-header').removeClassPrefix('header-');
            $('.pcoded-header').addClass(temp);
            $('.m-header > .b-brand > .logo').attr('src', '/static/assets/images/logo.png');
        }
    });
    // rtl layouts
    $('#theme-rtl').change(function() {
        ...
        if ($(this).is(":checked")) {
            $('.rtl-css').attr("href", "/static/assets/css/layout-rtl.css");
        } else {
            $('.rtl-css').attr("href", "");
        }
    });
